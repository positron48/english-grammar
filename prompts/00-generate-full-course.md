...
3. **Коммит после каждой главы** - **ОБЯЗАТЕЛЬНО** закоммить все изменения после завершения генерации каждой главы. Это критически важно для отслеживания прогресса и возможности восстановления после сбоя
4. **Проверка перед генерацией** - если глава уже сгенерирована (`status: "generated"` или `"validated"`), пропусти её
5. **Использование существующих входных данных** - если файл уже существует в `config/chapter-templates/`, используй его
6. **Следуй порядку** - генерируй главы в порядке `order` из status файла
7. **Учитывай prerequisites** - при генерации плана главы учитывай, какие главы должны быть пройдены до неё
8. **Отслеживание прогресса** - регулярно обновляй `summary` в status файле
