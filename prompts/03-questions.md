...
- Для `reorder`: строка `correct_answer` (полное предложение)
  ...
  - Не более 10 слов. Перевод и короткое ограничение в `prompt` должны задавать единственный допустимый порядок; при свободном порядке слов используйте `fill_blank` или `mcq_single` по тому же правилу.
  ...
  - Примеры:
    - Правильно: `"Соберите предложение. Начните с I, выражение времени поставьте в конец."` → correct_answer: "I go to school every day.", translation_ru: "Я хожу в школу каждый день."
    - Неправильно: `"Расставьте слова в правильном порядке: go, I, school, to"` → список слов не нужен даже без квадратных скобок
...
- Для `error_spotting`: массив `choices` с вариантами исправления
  - **Минимум 4 варианта:** 1 правильный + не менее 3 неправильных. Вопросы только с 2–3 вариантами — исправлять.
  - **Промпт без предложения:** если задание «выберите правильный вариант предложения» (или «краткий ответ» и т.п.), само предложение в текст промпта не включать — оно только в вариантах ответа. Исключение: «Найдите ошибку в предложении: «…»» с вариантами вида «X → Y» (конкретная правка) — предложение в промпте оставить.
  - **Формулировка промпта:** если варианты — целые предложения (правильное и неправильные), не используй голое «Найдите ошибку в структуре предложения» или «Найдите ошибку:» — непонятно, что выбирать. Используй: **«Выберите правильный вариант предложения.»** или **«Найдите ошибку и выберите исправленное предложение:»** / **«…корректный вариант»** / **«…лучшую правку»**.
  - **Варианты ответов:** в `choices` либо **готовые предложения** (1 правильное и ≥3 неправильных), либо **конкретные ошибки с исправлениями** (напр.: «Порядок слов: подлежащее перед сказуемым → He plays football»; тогда тоже ≥4 варианта).
